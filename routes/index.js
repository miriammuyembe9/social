...
// ── POSTS ─────────────────────────────────────────────────────────────────────
// NOTE: /posts/search and /posts/trending MUST come before /posts/:id
router.get   ('/posts/search',           auth, postCtrl.searchPosts);
router.get   ('/posts/trending',         auth, postCtrl.getTrending);          // ← NEW (algorithm)
...
router.post  ('/posts/:id/interact',     auth, postCtrl.recordInteraction);    // ← NEW (algorithm)
...
router.delete('/chat/messages/:id',             auth,                         mainCtrl.deleteMessage);
...
router.delete('/videos/:id',      auth, mainCtrl.deleteVideo);

// ── STORIES ───────────────────────────────────────────────────────────────────
router.get   ('/stories',          auth, upload.fields([{name:'media',maxCount:1},{name:'audio',maxCount:1}]), mainCtrl.getStories);
...
router.get('/debug/posts', auth, async (req, res) => {
  ...
  try {
    ...
    res.json({ test1_data: t1.data, test1_error: t1.error, test2_count: t2.count, test2_error: t2.error, user_id: req.user.id });
